...
export default class BLE extends Component {
  ...
  send = (msg = 'Test Object, 20, B17CS024, IIT Jodhpur') => {
    ...
    BleManager.write(peripheral.id, service, sendCharacterstic, sendData).then(
      data => {
        BleManager.write(
          peripheral.id,
          service,
  ...
  receive = () => {
    ...
    BleManager.retrieveServices(peripheral.id).then(peripheralInfo => {
      ...
      BleManager.read(peripheral.id, service, receiveCharacterstic).then(
        data => {
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.container}>
          ...
          <FormView>
            ...
            <FormButton
              title="Send"
              onFormSubmit={() => this.sendDataToServer(this.state.msg)}
            />
          </FormView>
          ...
          <FormView>
            ...
            {selectedDevice.name != undefined && (
              <View>
                ...
                <FormButton
                  title="Send"
                  onFormSubmit={() => this.send(this.state.msg)}
                />
                <FormButton
                  title="Receive"
                  onFormSubmit={() => this.receive()}
                />
